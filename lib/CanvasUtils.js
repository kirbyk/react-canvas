 ...
 *
 * @param {CanvasContext} ctx
 * @param {Number} xPos circle's center x coordinate
 * @param {Number} yPos circle's center y coordinate
 * @param {Number} radius circle's radius
 * @param {String} color circle's color
 */
function drawCircle(ctx, xPos, yPos, radius, color) {
  const path = new Path2D();
  path.arc(xPos, yPos, radius, 0, Math.PI * 2, true);
  ctx.fillStyle = color;
  ctx.fill(path);
}
 ...
 *
 * @param {CanvasContext} ctx
 * @param {Number} xPos rectangle's center x coordinate
 * @param {Number} yPos rectangle's center y coordinate
 * @param {Number} width rectangle's width
 * @param {Number} height rectangle's height
 * @param {String} color rectangle's color
 */
function drawRect(ctx, xPos, yPos, width, height, color) {
  const path = new Path2D();
  path.rect(xPos, yPos, width, height);
  ctx.fillStyle = color;
  ctx.fill(path);
}
