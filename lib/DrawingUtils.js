...
function drawRenderLayer (ctx, layer) {
  ...
  switch (layer.type) {
    ...
    case 'circle':
      customDrawFunc = drawCircleRenderLayer;
      break;
 ...
 * @private
 */
function drawCircleRenderLayer (ctx, layer) {
  var xPos = layer.xPos || layer.frame.width / 2;
  var yPos = layer.yPos || layer.frame.height / 2;
  var radius = layer.radius || 1;
  var color = layer.color || '#000000';
  CanvasUtils.drawCircle(ctx, xPos, yPos, radius, color);
}
...
function drawRectRenderLayer (ctx, layer) {
  ...
  var color = layer.color || '#000000';
  CanvasUtils.drawRect(ctx, xPos, yPos, width, height, color);
}
